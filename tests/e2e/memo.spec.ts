...
test('dispute wizard manual path marks findings pursued', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Continue', exact: true }).click()
  await expect(page.getByText('Your dispute is ready to send')).toBeVisible()
  await page.getByRole('button', { name: 'I sent the dispute' }).click()
  await expect(page.getByText('Variance groups marked pursued with QuickBox.')).toBeVisible()
